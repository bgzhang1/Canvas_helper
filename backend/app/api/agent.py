...
from ..runtime import get_agent_settings, state
...
async def agent_chat(payload: AgentChatIn) -> dict[str, Any]:
    ...
    agent_settings = get_agent_settings(include_secrets=True)
    if not agent_settings["base_url"] or not agent_settings.get("api_key"):
        state().db.add_event(
            category="agent",
        ...
        return {
            "role": "assistant",
            "content": "Agent model provider is not configured. Set base URL and API key in Settings first.",
            "tools_used": [],
    ...
    try:
        run = await agent.run(
            ...
            user_payload={"message": message, "history": history, "context": context},
            tools=tools,
    ...
    return {"role": "assistant", "content": content, "tools_used": run.tools_used, "status": "ok"}
...
async def _agent_chat_stream_events(payload: AgentChatIn, message: str):
    ...
    yield event_line(
        {
            "type": "done",
            "message": {"role": "assistant", "content": content, "tools_used": tools_used, "status": "ok"},
            "tools_used": tools_used,
        }
    )
