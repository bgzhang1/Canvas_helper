...
def test_telegram_notification_uses_configured_bot(tmp_path) -> None:
    requests: list[dict] = []
    ...
    result = service.send_telegram_message("Reminder: Lab 1 is due tomorrow.")
...
def test_email_reminder_writes_outbox_without_smtp(tmp_path) -> None:
    db = Database(tmp_path / "test.db")
    db.init()
    service = NotificationService(
        db,
    ...
    result = service.send_email_reminder(
...
def test_disabled_notification_channels_return_disabled(tmp_path) -> None:
    service = NotificationService(
        None,
        NotificationConfig(
    ...
    assert service.send_telegram_message("hello")["status"] == "disabled"
    assert service.send_email_reminder(subject="Reminder", body="Body")["status"] == "disabled"
