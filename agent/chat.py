...
Kept out of the API layer so the HTTP router stays thin.
"""
...
def _search_course_materials(args: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    def run(query: str, course: str) -> list[dict[str, Any]]:
        search = getattr(state().db, "search_course_materials", None)
        rows = search(query, course=course, limit=limit) if callable(search) else []
